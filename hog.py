...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while rolls_made < num_rolls:
        roll_outcome = dice()
        if roll_outcome == 1:
            pig_out = True 
        hog_total += roll_outcome
        rolls_made += 1

    return 1 if pig_out else hog_total
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        if who == 0:
            ...
            turn_score = take_turn(num_rolls, score, dice)
            opponent_score += turn_score

        # this is where ill put swine swap
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    highest_average = 0
    highest_num_rolls = 1   
    num_samples = 1000 # im keeping it the same value from the last function
    num_rolls = 1  

    while num_rolls <= 10:
        averaged_roll = make_averaged(roll_dice, num_samples)
        average_score = averaged_roll(num_rolls, dice)
        print(f"{num_rolls} dice scores {average_score:.1f} on average") #using fstring because 
        # .format() is messy
...
def bacon_strategy(score, opponent_score):
    ...
    0
    """
    tens_decimal = opponent_score//10
    ones_decimal = opponent_score % 10 

    bacon_points = 1 + max(tens_decimal, ones_decimal)
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    current_bacon_pts = 1 + max(opponent_score//10, opponent_score % 10)
    new_score = score + current_bacon_pts

    if new_score == 2 * opponent_score:
        return BASELINE_NUM_ROLLS
    elif opponent_score == 2 * new_score:
        return 0 #do this because the swap benefits player
    
    if current_bacon_pts >= BACON_MARGIN:
        return 0
    else:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    *** my strategy uses swap, bacon, and hog wild to get a win 
    rate of 0.58. To calculate the decision you should make, the program
    checks how far behind you are or how ahead you are from the opponent to play safe
    or play risky for more points.  ***
    """
    bacon_points = 1 + max(opponent_score//10, opponent_score % 10)
    new_score = score + bacon_points

    # this is where i will use swine swap
    if opponent_score == 2*new_score:
        return 0
    if new_score == 2 * opponent_score:
        return BASELINE_NUM_ROLLS

    # try to use free bacon rule to get ahead of opponent
    if bacon_points >= BACON_MARGIN:
        return 0

    # checking if hog wild would activate to decide what action to do 
    if (score + opponent_score) % 7 == 0:
        return 4

    # this is where i add decision based on how far we are from opponent
    scoreDifference = score - opponent_score

    if scoreDifference > 10: # using 10 as a "big difference" between p1 & p2
        return 4
    elif scoreDifference < -10:
        return 6
    elif score > 90:
        return 3 # i play safer the closer i get to winning
    elif score > 95:
        return 2 # play even less risky to maybe help w/ win rate
    else:
        return BASELINE_NUM_ROLLS
